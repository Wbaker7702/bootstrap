...
Improve readability by suggesting better line breaks.

- `.text-balance`: Uses `text-wrap: balance` to distribute words more evenly across lines—ideal for headings.
- `.text-pretty`: Uses `text-wrap: pretty` to avoid orphaned words and improve rag balance in paragraphs.

<Example code={`<p class="fs-3 text-balance" style="max-width: 26rem;">
  A long headline designed to demonstrate balanced line wrapping behavior
</p>`} />

<Example code={`<p class="text-pretty" style="max-width: 26rem;">
  This paragraph demonstrates pretty wrapping that reduces orphan words and creates a more polished rag edge across multiple lines of text.
</p>`} />

<Callout type="warning">
`text-wrap: pretty` is currently implemented only in Chromium-based browsers and isn’t supported in Firefox or Safari. Treat `.text-pretty` as progressive enhancement—fall back to normal wrapping or `.text-balance`, or feature‑detect or browser‑detect before relying on it (for example `CSS.supports('text-wrap', 'pretty')`). See [MDN compatibility data](https://developer.mozilla.org/en-US/docs/Web/CSS/text-wrap#browser_compatibility).
</Callout>
