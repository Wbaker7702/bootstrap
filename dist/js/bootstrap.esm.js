...
function normalizeParameters(originalTypeEvent, handler, delegationFunction) {
  ...
  // Tooltip passes `false` instead of selector, so we need to check
 ...
 */
_Alert = Alert;
Alert.jQueryInterface = getjQueryInterface(_Alert, {
  passElement: true
});
...
class Carousel extends BaseComponent {
  ...
  _getOrderFromKey(key) {
    if (key !== ARROW_LEFT_KEY$1 && key !== ARROW_RIGHT_KEY$1) {
      return null;
    }
    const isLeftKey = key === ARROW_LEFT_KEY$1;
  ...
  _getDirectionFromKey(key) {
    ...
    if (isRTL()) {
      return key === ARROW_LEFT_KEY$1 ? DIRECTION_LEFT : DIRECTION_RIGHT;
    }
    return key === ARROW_LEFT_KEY$1 ? DIRECTION_RIGHT : DIRECTION_LEFT;
  }
 ...
 */
_Collapse = Collapse;
Collapse.jQueryInterface = getjQueryInterface(_Collapse, {
...
class Dropdown extends BaseComponent {
  constructor(element, config) {
    ...
    // v6 plan: revert #37011 & change markup https://getbootstrap.com/docs/5.3/forms/input-group/
  ...
  _getPopperConfig() {
    ...
    if (this._inNavbar || this._config.display === 'static') {
      Manipulator.setDataAttribute(this._menu, 'popper', 'static'); // v6 plan: remove
 ...
 */
_Dropdown = Dropdown;
Dropdown.jQueryInterface = getjQueryInterface(_Dropdown);
 ...
 */
_Modal = Modal;
Modal.jQueryInterface = getjQueryInterface(_Modal);
...
Offcanvas.jQueryInterface = getjQueryInterface(_Offcanvas, {
  passElement: true
});
...
const DATA_BS_ATTRIBUTE_PATTERN = /^data-bs-[\w-]*$/i;
const DefaultAllowlist = {
  // Global attributes allowed on any supplied element below.
  '*': ['class', 'dir', 'id', 'lang', 'role', ARIA_ATTRIBUTE_PATTERN, DATA_BS_ATTRIBUTE_PATTERN],
 ...
 */
_Tooltip = Tooltip;
Tooltip.jQueryInterface = getjQueryInterface(_Tooltip);
 ...
 */
_Popover = Popover;
Popover.jQueryInterface = getjQueryInterface(_Popover);
...
const Default$1 = {
  offset: null,
  // Deprecated; retained for backward compatibility until v6
...
ScrollSpy.jQueryInterface = getjQueryInterface(_ScrollSpy);
...
const SELECTOR_DATA_TOGGLE = '[data-bs-toggle="tab"], [data-bs-toggle="pill"], [data-bs-toggle="list"]'; // v6 plan: could be only `tab`
...
class Tab extends BaseComponent {
  constructor(element) {
    ...
    if (!this._parent) {
      return;
      // In v6, consider throwing to enforce valid parent
 ...
 */
_Tab = Tab;
Tab.jQueryInterface = getjQueryInterface(_Tab);
 ...
 */
_Toast = Toast;
Toast.jQueryInterface = getjQueryInterface(_Toast, {
  passElement: true
});
...
export { Alert, Button, Carousel, Chip, Collapse, Dropdown, Modal, Offcanvas, Popover, ScrollSpy, Tab, Toast, Tooltip };
